...
def run_git_command(path, command):
    ...
    try:
        ...
        print_status(f"Error executing git command: {e}", "error")
        return None
...
def get_git_info(path):
    ...
    try:
        ...
        git_info['last_commit_date'] = datetime.fromtimestamp(
        ...
        print_status(f"Error getting git info: {e}", "error")
...
def scan_directory(path):
    ...
    try:
        ...
        print_status(f"Error scanning directory: {e}", "error")
        return None
    ...
    try:
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            if git_info:
                ...
                f.write(
                    f"Last Commit Message: {git_info.get('last_commit_message', 'N/A')}\\n"
                )
                f.write(
                    f"Last Commit Date: {git_info.get('last_commit_date', 'N/A')}\\n"
                )
        ...
        print_status(f"Error creating index file: {e}", "error")
        return False
...
def main():
    ...
    if git_info:
        print_status("Git repository information collected successfully", "success")
    else:
        print_status(
            "No git repository found or error accessing git information",
            "warning"
        )
    
    # Scan directory
    print_status("Scanning directory structure...")
    structure = scan_directory(dir_path)
    if not structure:
        print_status("Error scanning directory", "error")
        return
    ...
    if create_index_file(structure, git_info, output_file):
        print_status(
            f"Index file created successfully at: {output_file}",
            "success"
        )
    else:
        print_status("Failed to create index file", "error")
